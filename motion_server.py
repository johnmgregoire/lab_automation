...
async def move_live(x_mm: float, axis: str, speed: int = None, mode: move_modes = 'relative'):
    """Move a apecified {axis} by {x_mm} distance at {speed} using {mode} i.e. relative"""
    #http://127.0.0.1:8001/motor/set/move?x_mm=-20&axis=x
    return StreamingResponse(motion.motor_move_live(x_mm, axis, speed, mode),
...
def query_positions():
    ...
    retc = return_class(measurement_type = 'motion_query',
...
def analog_in(port: int):
    ...
    retc = return_class(measurement_type = 'io_command',
