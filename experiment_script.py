...
for sno,dx,dy in zip([i for i in range(len(x))],x,y):
    ...
    pulse_exp = actions.pulse(20,10**-5,make_pulse(centers,pots,widths),blockd=blockd)
    ...
    exp_results[sno] = {'pulse_params':{'centers':centers, 'pots':pots, 'widths':widths},
                        'eis_params':{'start_freq':start_freq,'end_freq':end_freq,'points':points},
                        'eis_results':eis_exp,
